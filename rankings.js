...
function renderStats() {
  animateNumber(
    document.getElementById("totalRankedCount"),
    rankings.length
  );
...
function renderRankings() {
  ...
  const filtered = rankings.filter((ranking) => {
    const leagueName =
      ranking.leagues?.name?.toLowerCase() || "";

    if (
      rankingSearch &&
      !leagueName.includes(rankingSearch)
    ) {
      return false;
    }
  ...
  body.innerHTML = filtered.map((ranking, index) => {
...
function setupFilters() {
  document
    ...
    .addEventListener("input", (event) => {
      rankingSearch = event.target.value
        .trim()
        .toLowerCase();
...
function initializeAnimations() {
  ...
  document
    .querySelectorAll(".reveal-row:not(.is-visible), .section-heading:not(.is-visible)")
